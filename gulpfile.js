...
gulp.task("css-my-style", () =>
  gulp
    .src(["css/my-styles/main.scss"])
    .pipe(sass())
    .pipe(autoprefixer())
    .pipe(minify({ compatibility: "ie9" }))
    .pipe(header(banner))
    .pipe(gulp.dest("./dist/my-styles"))
);

gulp.task("css", gulp.parallel("css-my-style", "css-themes", "css-core"));
...
gulp.task("serve", () => {
  ...
  gulp.watch(
    ...
    gulp.series("css-my-style", "css-core", "reload")
  );
